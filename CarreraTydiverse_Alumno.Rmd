...
```{r}
# Asumiendo que mydf ya está cargado en tu sesión de R como se indica en la parte anterior
# Transforma mydf en un tibble y lo almacena en cran
cran <- as_tibble(mydf)

# Borra el data frame original mydf
rm(mydf)
...
```{r}
# Seleccionamos las columnas específicas
cran %>% 
...
cran %>%
  group_by(country) %>%
  summarise(total_size = sum(size, na.rm = TRUE)) %>%
  mutate(percentage = total_size / sum(total_size)) %>%
  filter(percentage > 0.9) %>%
  arrange(desc(total_size)) %>%
  ggplot(aes(x = reorder(country, total_size), y = total_size)) +
  geom_col() +
  labs(x = "Country Code", y = "Traffic (Total Size of Downloads)") +
  theme_minimal()
